...
class SolanaService {
   ...
   * 查询交易记录
   * @param publicKey - 钱包公钥
   * @param limit - 返回的交易数量限制
   * @returns 交易记录数组
   */
  async getTransactions(publicKey: string, limit = 10): Promise<Array<{
    signature: string;
      ...
      const transactions = await Promise.all(
        signatures.map(async (signature) => {
          ...
          return {
            ...
            slot: signature.slot,
            memo: (tx?.transaction?.message as any)?.instructions?.[0]?.parsed?.info?.memo || null,
   ...
   */
  async getAllTokenBalances(walletAddress: string): Promise<Record<string, {
    amount: string;
    decimals: number;
    uiAmount: number;
    symbol?: string;
      ...
      for (const account of tokenAccounts.value) {
        try {
          ...
          try {
            ...
            mint = accountData?.value?.data && typeof accountData.value.data === 'object' && 'parsed' in accountData.value.data
              ? (accountData.value.data as {parsed: {info: {mint: string}}}).parsed.info.mint
              : undefined;
            if (!mint) {
          ...
          try {
            // 尝试获取代币元数据
            const tokenInfo = await this.connection.getParsedAccountInfo(new PublicKey(mint));
            if (tokenInfo?.value?.data && typeof tokenInfo.value.data === 'object' && 'parsed' in tokenInfo.value.data) {
              const parsedData = tokenInfo.value.data as {parsed: {info: {symbol?: string}}};
              if (parsedData.parsed.info.symbol) {
            ...
            if (error instanceof Error) {
              ...
              console.log(`Could not get token info for ${mint}:`, String(error));
            }
          }
