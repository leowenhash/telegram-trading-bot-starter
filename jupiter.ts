...
async function getJupiterUltraOrder({inputMint, outputMint, amount, taker}: JupiterOrderParams) {
  try {
 ...
 */
async function executeJupiterUltraOrder(signedTransaction: string, requestId: string) {
  try {
    const response = await axios({
      method: 'post',
      maxBodyLength: Infinity,
 ...
 */
async function getJupiterUltraBalances(walletAddress: string) {
  try {
    const response = await axios({
      method: 'get',
      maxBodyLength: Infinity,
